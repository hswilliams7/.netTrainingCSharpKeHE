...
namespace HangMan.cs.hange_man
{
    class HangManGame
    {   // checking for word 
        private const string Value = "Oops, that letter is not in my word";
        ...
        static void Main()
        {
            ...
            Console.WriteLine("Your first task is to Guess a {0} letter Word ", secretword.Length);
            Console.ForegroundColor = ConsoleColor.Magenta;
            Console.WriteLine("You have {0} lives...GO!", lives);
            Isletter(secretword, letterGuessed);
            // while lives is greater than 0
            while (lives > 0)
            {
